"""
scripts/train_adaln_energy.py
==============================
Training script for AdaLN Transformer + energy CFG (DDPM).

Usage:
    python scripts/train_adaln_energy.py --config configs/transformer_adaln_energy.yaml
    python scripts/train_adaln_energy.py --config configs/transformer_adaln_energy.yaml \
        --resume checkpoints/transformer_adaln_energy/v1/latest.pt

model_type must be 'transformer_adaln_energy'.

Energy conditioning
-------------------
Each batch's raw energies are z-score normalised using statistics computed
from the full training set at startup.  The normalised values are passed to
the model as energy_z.  Stats are stored in config so checkpoints are
self-contained and sampling scripts don't need to re-scan the dataset.

CFG guidance at inference
--------------------------
Run the model twice per step:
    eps_cond   = model(x_t, t, energy_z=e_z)
    eps_uncond = model(x_t, t, energy_z=None)
    eps        = eps_uncond + guidance_scale * (eps_cond - eps_uncond)

See scripts/analyze_energy_conditioning.py for a full sweep.
"""
...
# ── Versioning (identical to train.py) ───────────────────────────────────────
...
def build_model(config: dict) -> nn.Module:
    mc = config['model']
    assert config['model_type'] == 'transformer_adaln_energy', \
        f"This script only supports model_type='transformer_adaln_energy', got {config['model_type']!r}"
    return AdaLNEnergyTransformerScoreNetwork(
...
def train(config: dict, resume_path: str = None):
    ...
    if resume_path:
        ...
        # restore energy stats if checkpoint has them
    ...
    for epoch in range(start_epoch, tc['n_epochs']):
        ...
        for batch in pbar:
            ...
            e_raw = batch['energies'].to(device)         # (B,)
            e_z   = (e_raw - energy_mean) / energy_std   # z-normalised

            # inline diffusion loss so we can pass energy_z directly
            B = x0.shape[0]
